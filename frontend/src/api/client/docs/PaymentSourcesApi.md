...
```typescript
import { PaymentSourcesApi, Configuration } from "./api";
...
const { status, data } =
  await apiInstance.paymentSourcesControllerArchivePaymentSource(id);
```
...
const { status, data } =
  await apiInstance.paymentSourcesControllerCreatePaymentSource(body);
```
...
const { status, data } =
  await apiInstance.paymentSourcesControllerListPaymentSources();
```
...
const { status, data } =
  await apiInstance.paymentSourcesControllerUpdatePaymentSource(id, body);
```
