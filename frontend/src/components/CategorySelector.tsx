...
function buildVisibleItems(categories: Category[], search: string): VisibleItem[] {
  ...
  for (const parent of categories) {
    ...
    items.push({ id: parent.id, name: parent.name, isParent: true, displayPath: parent.name })
              ...
              visibleItems.map((item, index) => (
                <li
                  ...
                  className={cn(
                    ...
                    item.isParent ? 'px-3 font-semibold' : 'pr-3 pl-8 font-normal',
                  )}
                >
